...
export default function DuolingoButton({
  ...
  const variantStyles = {
    ...
    disabled: "bg-[#E5E5E5] text-[#AFAFAF] border-b-2 border-[#CCCCCC] cursor-not-allowed shadow-[0_3px_0_#CCCCCC]",
  ...
  const variantStyle = disabled || loading ? variantStyles.disabled : variantStyles[variant]
...
function LoadingSpinner({ variant }: { variant: string }) {
  const spinnerColor = variant === "secondary" || variant === "dashedOutline" ? "text-indigo-600" : "text-white"
